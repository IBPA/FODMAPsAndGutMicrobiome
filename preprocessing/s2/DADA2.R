...
fnFs <- sort(list.files(src_dir, pattern="ERR.*_1.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(src_dir, pattern="ERR.*_2.fastq.gz", full.names = TRUE))
...
filtRs <- file.path(src_dir, "filtered", paste0(sample.names, "_R_filt.fastq.gz"))
names(filtFs) <- sample.names
names(filtRs) <- sample.names

# Filter and Trim
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, , truncLen=c(260,260), trimLeft = c(16, 16),
