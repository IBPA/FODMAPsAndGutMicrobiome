...
this_get_sample_filenames <- function(src_dir) {
  ...
  fnFWs <- sort(list.files(src_dir, pattern=".*_1.fastq.gz", full.names = TRUE))
  fnREVs <- sort(list.files(src_dir, pattern=".*_2.fastq.gz", full.names = TRUE))
...
out <- filterAndTrim(sfns$FWs, sfns$filtFWs,
                     truncLen=250, trimLeft = 25,
